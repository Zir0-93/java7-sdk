...
public abstract class Signature extends SignatureSpi {
    ...
    public static Signature getInstance(String algorithm)
        ...
        if (algorithm.equalsIgnoreCase(RSA_SIGNATURE)) {
            ...
            services = GetInstance.getServices("Signature", algorithm);
        }
        // try services until we find an Spi or a working Signature subclass
        NoSuchAlgorithmException failure = null;
        for (Service s: services) {
            if (isSpi(s)) {
                return new Delegate(s, services, algorithm);
            } else {
        ...
        throw new NoSuchAlgorithmException
            ("Algorithm " + algorithm + " not available", failure);
    }
    ...
    private static class Delegate extends Signature {
        ...
        // The selected provider implementation (delegate)
        ...
        // first service to try in provider selection
        private final Service firstService;

        // services to try in provider selection
        ...
        Delegate(Service s, Iterable<Service> t, String algorithm) {
            super(algorithm);
            this.firstService = s;
            this.services = t;
        ...
        void chooseFirstProvider() {
            ...
            synchronized (lock) {
                ...
                boolean skipping = (firstService != null);
                for (Service s: services) {
                    if (skipping) {
                        if (s != firstService) {
                            continue;
        ...
        private void chooseProvider(int type, Key key, SecureRandom random)
            ...
            synchronized (lock) {
                if ((sigSpi != null) && 
                        ((service == null) || service.supportsParameter(key))) {
        ...
        protected void engineInitVerify(PublicKey publicKey)
                throws InvalidKeyException {
            if ((sigSpi != null) && ((service == null) ||
                                     service.supportsParameter(publicKey))) {
        ...
        protected void engineInitSign(PrivateKey privateKey)
                throws InvalidKeyException {
            if ((sigSpi != null) && ((service == null) || 
                                     service.supportsParameter(privateKey))) {

...
public class BigInteger extends Number implements Comparable<BigInteger>,
    ...
    /** number of bits in the binary representation, used for serialization */
    private int bitCount;

    /** first no zero digit, used for serialization*/
    private int firstNonzeroByteNum;

    /**
     * the length of the minimal two's complement representation, used for
     * serialization
     */
    private int bitLength;

    /** position of lowest bit, used for serialization */
    ...
    private static void setFromString(BigInteger bi, String val, int radix) {
        ...
        int minusIndex = val.lastIndexOf('-');
        int plusIndex = val.lastIndexOf('+');
        // Check for leading sign
        // both '+' or '-' not found or only one of them found
        if ((minusIndex + plusIndex) <= -1) {
            // only have '+' or '-' but no numbers
            if ((minusIndex == 0 || plusIndex == 0) && stringLength == 1) {
    ...
    public int hashCode() {
        ...
        //CMVC181711 use the local variable to make hashCode() method thread safe
        int h = 0;
        //CMVC180417 digits array may be have additional trailing zeros, but
        //hashCode should be same if 2 BigIntegers are equal.
        int lastNonZeroIndex = digits.length;
        while (lastNonZeroIndex > 0 && digits[--lastNonZeroIndex] == 0) {
        }
        for (int i = 0; i <= lastNonZeroIndex; i++) {
            h = (h * 33 + (digits[i] & 0xffffffff));
        }
        hashCode = h * sign;
        return hashCode;
    }
    ...
    public BigInteger modPow(BigInteger exponent, BigInteger m) {
        ...
        //CMVC177426 workaround
        //Let m = q * 2^j, if j >=31, use normal mod arithmetic, otherwise
        //middle BigInteger object will exceed the limit of BigInteger internal representation
        int j = m.getLowestSetBit();
        if (j >=31) {
            if (exponent.compareTo(maxInt) <= 0) {
                int expValue = exponent.intValue();
                BigInteger base = pow(expValue);
                return base.mod(m);
            } else {
    ...
    private void readObject(ObjectInputStream in) throws IOException,
        ...
        if (magnitude.length == 0) {
            if (signum != 0) {
                // math.14=signum-magnitude mismatch
                throw new StreamCorruptedException(Messages.getString("math.14")); //$NON-NLS-1$
            }
            sign = 0;
            numberLength = 1;
            digits = new int[] { 0 };
    ...
    private void writeObject(ObjectOutputStream out) throws IOException {
        bitCount = bitCount();
        firstNonzeroByteNum = getFirstNonzeroDigit();
        bitLength = bitLength();
        lowestSetBit = getLowestSetBit();
        signum = signum();
        // avoid assigning magnitude field multiple times, which could cause problem
        // in multi-thread environment.
        byte[] magArray = abs().toByteArray();
        magnitude = (sign == 0) && (magArray.length != 0) ? new byte[0] : magArray;

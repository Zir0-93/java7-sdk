/*
 *  2012-01-27: this file was modified by International Business Machines Corporation.
 *  Modifications Copyright 2012 IBM Corporation.
 */
...
class Logical {
    ...
    static BigInteger not(BigInteger val) {
        ...
        if (val.sign > 0) {
            ...
            if (val.digits[val.numberLength - 1] != -1) {
                ...
                if (i == val.numberLength) {
                    resDigits[i] = 1;
                    result = new BigInteger(-val.sign, i + 1, resDigits);
        ...
        result = new BigInteger(-val.sign, i, resDigits);
        result.cutOffLeadingZeroes();
        return result;
    }
    ...
    /**
     * This method completes the ^ (xor) operation with the two's-complement
     * representation. Since the internal representation with int[] is an
     * unsigned integer representation, the process of this method is:
     * <p>
     * 1. convert the internal unsigned representation to two's-complement
     * representation, positive and negative all need an extra element to
     * represent the sign values so the 2 big integers need expand the int[]
     * (the internal representation) with length as original length + 1.
     * <p>
     *     1.1 for the positive, the extra element positive.digits[original length]
     * will be 0x0000 0000 for the negtive, the extra element
     * negtive.digits[original length] will be 0x1000 0000
     *
     *     1.2 complete 1.1, the positive.digits[] is already the two's-complement
     * representation, the negtive need be converted to the two's-complement
     * representation follow below algorithm:
     *         a. keep the sign value bit as it be (1)
     *         b. ~(the left bits) + 1 (~: nor operation)
     *
     * 1.3 take the notions of above converted integers as postive_tc and
     * negtive_tc
     *
     * <p>
     * 2. the two's-complement representation of the result of this methods is
     * result.digits[i] = positive_tc.digits[i] ^ negtive_tc. digits[i]
     *
     * <p>
     * 3. convert the result of step 2 to the BigInteger's internal unsigned
     * integer representation.
     *     3.1 Keep the sign value bit as it be (1), then
     * ~(other left bits) + 1
     *     3.2 Discard the extra element which contains the
     * sing value bit
     *     3.3 Complete above steps, the result BigInteger internal
     * representation will be:
     *         a. index < min (iNeg, iPos): result.digits[index]
     *         = 0
     *         b. when iPos < iNeg and index == iPos, result.digits[index] =
     *         -positive.digits[index]
     *             b.1 iPos < index < iNeg,
     *             result.digits[index] = ~(positive.digits[index])
     *             b.2 index == iNeg, result.digits[index] = ~(positive.digits[index] ^
     *             (~negtie.digits[index] + 1) ) -> = ~(positive.digits[index] ^
     *             -negtie.digits[index] ) (~a + 1 == -a)
     *             b.3 index > iNeg, result.digits[index] = ~(positive.digits[index] ^
     *             ~negtive.digits[index])
     * <p>
     * 4. step 3 only includes the condition of iPos < iNeg, will add other
     * conditions later
     *
     * @return sign = 1, magnitude = -(positive.magnitude ^ -negative.magnitude)
     * */
    static BigInteger xorDiffSigns(BigInteger positive, BigInteger negative) {
        ...
        if (iNeg < iPos) {
            ...
            // When the first non-zero digit of the negative is reached, must
            // apply
            ...
            if (i == iNeg) {
                ...
                // if the positive has no more elements must fill the remaining
                // digits with
                // the negative ones
                for (; i < iNeg; i++) {
                    // resDigits[i] = ~(0 ^ 0)
            ...
            int digit;
            // The first non-zero digit of the positive and negative are the
            // same
            i = iNeg;
            ...
            if (digit == 0) {
                ...
                if (digit == 0) {
                    ...
                    if (digit == 0) {
                        ...
                        BigInteger result = new BigInteger(-1, resLength,
                                resDigits);
                        return result;
                    }
                }

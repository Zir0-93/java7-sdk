...
public synchronized StringBuffer deleteCharAt(int location) {
	if (0 <= location && location < count) {
		...
		count--;
		return this;
	} else throw new StringIndexOutOfBoundsException(location);
}
	...
	if (start >= 0) {
		...
		if (end > start) {
			...
			if (diff > 0) { // replacing with fewer characters
				if (!shared) {
					System.arraycopy(value, end, value, start + stringLength, count - end); // index == count case is no-op
				} else {
					char[] newData = new char[value.length];
					System.arraycopy(value, 0, newData, 0, start);
					System.arraycopy(value, end, newData, start + stringLength, count - end); // index == count case is no-op
					value = newData;
					shared = false;
				}
			} else if (diff < 0) { // replacing with more characters... need some room
				move(-diff, end);
			}
			else if (shared) {
...
public synchronized StringBuffer reverse() {
	...
	if (!shared) {
		...
		for (int i = 0, mid = count / 2; i < mid; i++, --end) {
			...
			if (allowFrontSur && frontLow >= 0xdc00 && frontLow <= 0xdfff && frontHigh >= 0xd800 && frontHigh <= 0xdbff) {
				surAtFront = true;
				if (count < 3) return this;
			}
			if (allowEndSur && endHigh >= 0xd800 && endHigh <= 0xdbff && endLow >= 0xdc00 && endLow <= 0xdfff) {
				surAtEnd = true;
			}
			allowFrontSur = true;
			allowEndSur = true;
...
public synchronized void setCharAt(int index, char ch) {
	...
	if (0 <= index && index < count) value[index] = ch;
	else throw new StringIndexOutOfBoundsException(index);
}
...
public synchronized void setLength(int length) {
	if (length > value.length) {
	  ...
	  if (length < 0) throw new IndexOutOfBoundsException();
		...
		value = newData;
		shared = false;
	} else if (length < 0) throw new IndexOutOfBoundsException();
	count = length;
}
...
public synchronized String toString () {
	if (false == TOSTRING_COPY_BUFFER_ENABLED) {
		...
		if (wasted >= 768 || (wasted >= INITIAL_SIZE && wasted >= (value.length >> 1))) {
			return new String(value, 0, count);
		}
		shared = true;
		return new String (0, count, value);
	} else {
		/* Do not copy the char[] if it will not
		 * get smaller because of object alignment.
		 */
		int roundedCount = (count + 3) & ~3;
		if (roundedCount < value.length) {
			return new String(value, 0, count);
		}
		shared = true;
		return new String (0, count, value);
...
public synchronized StringBuffer append(StringBuffer sbuffer) {
	if (sbuffer == null) return append((String)null);
...
public synchronized int indexOf(String subString, int start) {
	...
	if (subCount > 0) {
		...
		while (true) {
			...
			if (!found || subCount + i > count) return -1; // handles subCount > count || start >= count
			int o1 = i, o2 = 0;
			while (++o2 < subCount && value[++o1] == subString.charAt(o2));
			if (o2 == subCount) return i;
			start = i + 1;
		}
	} else return (start < count || start == 0) ? start : count;
}
 ...
 * @exception	NullPointerException when squence is null
 *
 * @since 1.5
 */
	...
	if (start >= 0 && end >= 0 && start <= end && end <= sequence.length()) {
		if (sequence instanceof StringBuffer) {
			...
			synchronized(sequence) {
				return append(((StringBuffer)sequence).value, start, end - start);
			}
		} else if (sequence instanceof StringBuilder) {
			// optimize StringBuilder for speed
			return append(((StringBuilder)sequence).getValue(), start, end - start);
		}
		int len = end - start;
		if (len > 0) {
...
public synchronized StringBuffer insert(int index, CharSequence sequence) {
	if (index >= 0 && index <= count) {
		...
		if (sequence instanceof String) {
			...
			// optimize StringBuffer for synchronization, and speed
			synchronized(sequence) {
				return insert(index, ((StringBuffer)sequence).value, 0, ((StringBuffer)sequence).count);
			}
		} else if (sequence instanceof StringBuilder) {
			// optimize StringBuilder for speed
			StringBuilder builder = (StringBuilder)sequence;
	...
	if (index >= 0 && index <= count) {
		...
		if (start >= 0 && end >= 0 && start <= end && end <= sequence.length()) {
			if (sequence instanceof StringBuffer) {
				// optimize StringBuffer for synchronization, and speed
				synchronized(sequence) {
					return insert(index, ((StringBuffer)sequence).value, start, end - start);
				}
			} else if (sequence instanceof StringBuilder) {
				// optimize StringBuilder for speed
				return insert(index, ((StringBuilder)sequence).getValue(), start, end - start);
			}
			int min = end - start;
			if (min > 0) {
				move(min, index);

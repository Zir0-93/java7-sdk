...
class SecurityManager {
    ...
    private static class InterruptAccessLock {}                                 //IBM-javalock_obj_renaming
    private static final Object packageAccessLock = new InterruptAccessLock();  //IBM-javalock_obj_renaming
                                                                                //IBM-javalock_obj_renaming

    private static boolean packageDefinitionValid = false;
    private static PackageCollection packageDefinition;
    private static class InterruptDefinitionLock {}                             //IBM-javalock_obj_renaming
    private static final Object packageDefinitionLock = new InterruptDefinitionLock(); //IBM-javalock_obj_renaming
                                                                                //IBM-javalock_obj_renaming


    private static class PackageCollection {
        // if packages size is greater than or equal to THRESHOLD, use HashSet,
        // else use array as before. The THRESHOLD set to 50 is based on test with JIT on.
        public static int THRESHOLD = 50;
        boolean isSet;
        String[] packagesInArray;
        Set<String> packagesInSet;

        public PackageCollection(String[] packages) {
            isSet = false;
            packagesInArray = packages;
        }

        public PackageCollection(Set<String> packages) {
            isSet = true;
            packagesInSet = packages;
        }
    }

    private static PackageCollection getPackages(String p) {
        ...
        if (p != null && !p.equals("")) {
            ...
            if (n > 0) {
                                                                                //IBM-T6_wrt_bringup
                //packages = new String[n];                                     //IBM-T6_wrt_bringup
				...
				{	                                       //IBM-T6_wrt_bringup
                    ...
                    if (n >= PackageCollection.THRESHOLD) {
                        packages = new PackageCollection(new HashSet<String>(n));
                        // HashMap is lazy initialized, add one element to force
                        // HashMap create backing array in safe memory area
                        String s = tok.nextToken().trim();
                        packages.packagesInSet.add(s.intern());
                    }
                    else {
                        packages = new PackageCollection(new String[n]);
                    }
				}                                               //IBM-T6_wrt_bringup
                ...
                if (packages.isSet) {
                    ...
                    while (tok.hasMoreElements()) {
                        String s = tok.nextToken().trim();
                        packages.packagesInArray[i++] = s.intern();
                    }
                }
            }
        }
        ...
        if (packages == null)
        {                                                                       //IBM-T6_wrt_bringup
            //packages = new String[0];                                         //IBM-T6_wrt_bringup
            long oldMemory=-1;                                                  //IBM-T6_wrt_bringup
            try                                                                 //IBM-T6_wrt_bringup
			{                                                       //IBM-T6_wrt_bringup
				oldMemory = MemorySafetyService.enterSafeMemoryArea(); //IBM-T6_wrt_bringup
				packages = new PackageCollection(new String[0]);                       //IBM-T6_wrt_bringup
			}                                                       //IBM-T6_wrt_bringup
			finally                                                 //IBM-T6_wrt_bringup
			{                                                       //IBM-T6_wrt_bringup
				MemorySafetyService.exitLastMemoryArea(oldMemory); //IBM-T6_wrt_bringup
    ...
    public void checkPackageAccess(String pkg) {
        ...
        synchronized (packageAccessLock) {
            ...
            // changes afterwards; contents won't change.
            packageCollection = packageAccess;
        }

        /*
         * Traverse the packages, check for any matches.
         * If packages are in HashSet, check whether the package and its all
         * parent packages are in HashSet.
         * If packages are in array, go through the array to check whether the
         * package is start with each element or equals to it.
         * It is much faster to use HashSet when package.access list is long, 
         * but slower to use HashSet when package.access list is extremely short.
         * As package.access can be set by application code, keep the array code
         * path in case package.access is set to a very short list.
         */
        if (packageCollection.isSet) {
            String parentPackage = pkg;
            int lastDotIndex = pkg.length();
            while (lastDotIndex != -1)
            {
                parentPackage = parentPackage.substring(0, lastDotIndex);
                if (packageCollection.packagesInSet.contains(parentPackage + ".")) {
                    checkPermission(
                            new RuntimePermission("accessClassInPackage."+pkg));
                    break;  // No need to continue; only need to check this once
                }
                lastDotIndex = parentPackage.lastIndexOf('.');
            }
        }
        else {
            String[] pkgs = packageCollection.packagesInArray;
    ...
    public void checkPackageDefinition(String pkg) {
        ...
        synchronized (packageDefinitionLock) {
            ...
            packageCollection = packageDefinition;
        }

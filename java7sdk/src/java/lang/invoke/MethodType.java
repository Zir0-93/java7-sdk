...
public final class MethodType implements Serializable {
	...
	static final Set<Class<?>> WRAPPER_SET = Collections.unmodifiableSet(new HashSet<Class<?>>(Arrays.asList(wrappers)));
	static HashMap<String, Class> primitives = new HashMap<String, Class>();
	static final String primitiveChars = "BCDFIJSVZ"; //$NON-NLS-1$
	static {
		primitives.put("Z", boolean.class); //$NON-NLS-1$
		primitives.put("B", byte.class); //$NON-NLS-1$
		primitives.put("C", char.class); //$NON-NLS-1$
		primitives.put("S", short.class); //$NON-NLS-1$
		primitives.put("I", int.class); //$NON-NLS-1$
		primitives.put("F", float.class); //$NON-NLS-1$
		primitives.put("J", long.class); //$NON-NLS-1$
		primitives.put("D", double.class); //$NON-NLS-1$
	}
	...
	static class InternTableAddLock { }
	...
	private static final ArrayList<Class<?>> parseIntoClasses(String methodDescriptor, ClassLoader classLoader) {
		...
		if (signature[index] != '(') {
			throw new IllegalArgumentException("missing opening '(': " + methodDescriptor);
		}
		index++;
		...
		while((index < length) ) {
			...
			if ((current == ')')) {
				if (closeBracket) {
					throw new IllegalArgumentException("too many ')': " + methodDescriptor);
				}
				closeBracket = true;
				index++;
				continue;
			...
			if ((current == 'L') || (current == '[')) {
				...
				if (primitiveChars.indexOf(current) < 0) {
					throw new IllegalArgumentException("not a primitive: " + current);
				}
				if (current == 'V') {
					c = void.class;
				} else {
					c = primitives.get(Character.toString(current));
				}
			}
			args.add(c);
			index++;
		...
		synchronized(internTableAddLock) {
			...
			for(Class<?> c : arguments) {
				...
				if ((c == double.class) || (c == long.class)) {
					...
					throw new IllegalArgumentException("invalid parameter: " + void.class);
				}
			}
			if (stackSlots > 255) {
				throw new IllegalArgumentException("MethodType would consume more than 255 argument slots: " + stackSlots);
			}
			argSlots = stackSlots;
	...
	public List<Class<?>> parameterList(){
		List<Class<?>> list = Arrays.asList(arguments);
	...
	private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
		try {
			Field fReturnType = getClass().getDeclaredField("returnType"); //$NON-NLS-1$
			fReturnType.setAccessible(true);
			fReturnType.set(this, in.readObject());
			Field fArguments = getClass().getDeclaredField("arguments"); //$NON-NLS-1$

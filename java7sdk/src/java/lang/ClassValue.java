...
public abstract class ClassValue<T> {
    ...
    private static Entry<?>[] getCacheCarefully(Class<?> type) {
        ...
        // for mt, read classValueMap per tenant from slot
        ClassValueMap map;
        if (TenantGlobals.isTenantEnabled()) {
            // throw NPE if type is null, aligning with standard logic
            if (type == null) throw new NullPointerException();
            map = typeHashMap.get(type);
        } else {
    ...
    static WeakHashMap<Class<?>, ClassValueMap> typeHashMap = new WeakHashMap<Class<?>, ClassValueMap>();
    
    private static ClassValueMap initializeMap(Class<?> type) {
        ClassValueMap map = null;
        synchronized (CRITICAL_SECTION) {  // private object to avoid deadlocks
            // happens about once per type
            // The object leak issue 54701 happens when type is from root tenant but current tenant is app tenant.
            // then type.classValueMap = new ClassValueMap() will cause object leak issue.
            // to avoid this, use a tenant scope hashmap to store the ClassValueMap, then use this one when getMap().
            if (TenantGlobals.isTenantEnabled()) {
                // throw NPE if type is null, aligning with standard logic
                if (type == null) throw new NullPointerException();
                if ((map = typeHashMap.get(type)) == null) {
                    map = new ClassValueMap(type);
                    typeHashMap.put(type, map);
                }
            } else {

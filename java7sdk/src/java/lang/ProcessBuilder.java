...
public final class ProcessBuilder
{
    ...
    ProcessBuilder environment(String[] envp) {
        ...
        if (envp != null) {
            if (TenantGlobals.isTenantEnabled()) {                              //IBM-multitenancy_management
                environment = ProcessEnvironment.environment();                 //IBM-multitenancy_management
            } else {                                                            //IBM-multitenancy_management
                environment = ProcessEnvironment.emptyEnvironment(envp.length); //IBM-multitenancy_management
    ...
    public Process start() throws IOException {
        ...
        String dir = null;                                                     //IBM-multitenancy_management
        if (TenantGlobals.isTenantEnabled()) {                                 //IBM-multitenancy_management
            if (directory == null || !directory.isAbsolute()) {                //IBM-multitenancy_management
                String workDir = ProcessEnvironment.getenv(                     //IBM-multitenancy_management
                        "externalprocess.workingDirectory");                    //IBM-multitenancy_management
                if (workDir != null) {                                         //IBM-multitenancy_management
                    if (directory == null || directory.getPath().isEmpty()) {  //IBM-multitenancy_management
                        dir = workDir;                                          //IBM-multitenancy_management
                    } else {                                                   //IBM-multitenancy_management
                        dir = new File(workDir, directory.toString()).toString(); //IBM-multitenancy_management
                    }                                                           //IBM-multitenancy_management
                }                                                               //IBM-multitenancy_management
            } else {                                                           //IBM-multitenancy_management
                dir = directory.toString();                                     //IBM-multitenancy_management
            }                                                                   //IBM-multitenancy_management
        } else {                                                               //IBM-multitenancy_management
            dir = directory == null ? null : directory.toString();             //IBM-multitenancy_management
        }                                                                       //IBM-multitenancy_management

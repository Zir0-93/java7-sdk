...
public final class System {
	...
	static void afterClinitInitialization() {
		...
		try {
			sun.misc.VM.saveAndRemoveProperties(systemProperties);
		} catch (NoSuchMethodError e) {
		}

		/**
		 * When the System Property == true, then enable copy in
		 * String (String) constructor. Otherwise, disable copy.
		 */
		String enableCopyProperty = getProperty("java.lang.string.constructor.copy.substrings");
		if (enableCopyProperty != null) {
			String.enableCopy = enableCopyProperty.equalsIgnoreCase("true");
		}
...
static void runInitializationHook(String hookName) {
	...
	/**
	 * logically: com.ibm.misc.SystemIntialization.<hookName>(); however
	 * to enable sharing of this code by Java 7 RAW configurations which lack
	 * this class we'll use reflection.
	 */
	try {
		Class systemInitialization = Class.forName("com.ibm.misc.SystemIntialization"); //$NON-NLS-1$
		Method hook = systemInitialization.getMethod(hookName, null);
		hook.invoke(null, null);
		return;
	} catch (ClassNotFoundException e) {
		// Assume this is a Java 7 raw configuration and suppress the exception
		return;
	} catch (Exception e) {
		// Anything else is actually an error
...
static void completeInitialization() {
	...
	runInitializationHook("firstChanceHook"); //$NON-NLS-1$

	setIn(com.ibm.jvm.io.ConsoleInputStream.localize(new BufferedInputStream(new FileInputStream(FileDescriptor.in))));
	Terminator.setup();

	accessor = new Access();
	sun.misc.SharedSecrets.setJavaLangAccess(accessor);

	runInitializationHook("lastChanceHook"); //$NON-NLS-1$
...
private static void ensureProperties() {
	...
	systemProperties.put("java.version", "1.7.0");
	...
	systemProperties.put("com.ibm.oti.jcl.build", "20140627_204598");

	/* CMVC 152703 - get the J9 core/kernel class library version into the version string.
	 * the verson string gets its J9 component from the system property java.vm.info.  It has already been populated
	 * with VM, JIT and GC info, so here we concatenate the JCL drop info.
	 * We do the same for the java.fullversion property.
	 */
	...
	String info = systemProperties.getProperty("java.vm.info");
	if(info != null) {
		systemProperties.put("java.vm.info", info + thisVersion);
	}

	lineSeparator = getProperty("line.separator", null);
...
public static Properties getProperties() {
	if (!propertiesInitialized) throw new Error("bootstrap error, system Properties access before init");
	SecurityManager security = System.getSecurityManager();
	if (security != null)
...
static Properties internalGetProperties() {
	if (!propertiesInitialized) throw new Error("bootstrap error, system property access before init");
...
public static String getProperty(String prop, String defaultValue) {
	...
	if (!propertiesInitialized
			...
			&& !prop.equals("file.encoding.pkg")
			&& !prop.equals("sun.nio.cs.map")) {
		if (prop.equals("os.encoding")) {
			return osEncoding;
		} else if (prop.equals("ibm.system.encoding")) {
			return platformEncoding;
		} else if (prop.equals("file.encoding")) {
			return fileEncoding;
		}
...
public static String setProperty(String prop, String value) {
	if (!propertiesInitialized) throw new Error("bootstrap error, system property access before init: " + prop);

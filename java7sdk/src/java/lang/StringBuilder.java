	...
	if (start >= 0 && end >= 0 && start <= end && end <= sequence.length()) {
		if (sequence instanceof StringBuffer) {
			...
			synchronized(sequence) {
				return append(((StringBuffer)sequence).getValue(), start, end - start);
			}
		} else if (sequence instanceof StringBuilder) {
			// optimize StringBuilder for speed
			return append(((StringBuilder)sequence).value, start, end - start);
		}
		int len = end - start;
		if (len > 0) {
...
public StringBuilder insert(int index, CharSequence sequence) {
	if (index >= 0 && index <= count) {
		...
		if (sequence instanceof String) {
			...
			// optimize StringBuffer for synchronization, and speed
			synchronized(sequence) {
				return insert(index, ((StringBuffer)sequence).getValue(), 0, sequence.length());
			}
		} else if (sequence instanceof StringBuilder) {
			// optimize StringBuilder for speed
			StringBuilder builder = (StringBuilder)sequence;
	...
	if (index >= 0 && index <= count) {
		...
		if (start >= 0 && end >= 0 && start <= end && end <= sequence.length()) {
			if (sequence instanceof StringBuffer) {
				// optimize StringBuffer for synchronization, and speed
				synchronized(sequence) {
					return insert(index, ((StringBuffer)sequence).getValue(), start, end - start);
				}
			} else if (sequence instanceof StringBuilder) {
				// optimize StringBuilder for speed
				return insert(index, ((StringBuilder)sequence).value, start, end - start);
			}
			int min = end - start;
			if (min > 0) {
				move(min, index);

...
public final class String implements Serializable, Comparable<String>, CharSequence {
	...
	static {
		...
		seed = sun.misc.Hashing.murmur3_32(getSeed(), ascii, 0, ascii.length);
	}
...
public String (String string) {
	if (enableCopy && string.count != string.value.length) {
		if (string.count == 0) {
			...
			value = ascii;
			count = 0;
		} else if (string.count == 1 && string.value[string.offset] < 128) {
			offset = string.value[string.offset];
			value = ascii;
			count = 1;
		} else {
			offset = 0;
			value = new char[string.count];
...
public int compareTo (String string) {
	...
	while (o1 < end)
		if ((result = value[o1++] - target[o2++]) != 0) return result;
...
public int compareToIgnoreCase (String string) {
	...
	char[] target = string.value;
	while (o1 < end) {
		if ((c1 = value[o1++]) == (c2 = target[o2++])) continue;
		c1 = compareValue(c1);
		c2 = compareValue(c2);
...
/**
 * Returns an alternative 32-bit hash value for this String.
 */
int hash32() {
	if (hashCode32 == 0) {
		hashCode32 = sun.misc.Hashing.murmur3_32(seed, value, offset, count);
	}
		...
		if (codePoint != toLowerCase(codePoint)) {
			...
			if (!"tr".equals(language) && !"az".equals(language) && !"lt".equals(language)) { // not Turkish, Azeri, Lithuanian //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				while (o < count) {
					...
					if (codePoint >= Character.MIN_SUPPLEMENTARY_CODE_POINT) o++;
					o++;
				}
				...
				while (o < count) {
					...
					if (codePoint == 0x3a3) {
						builder.append(convertSigma(o));
					} else if (codePoint == 0x49 || codePoint == 0x4a || codePoint == 0x12e) { // I, J, I WITH OGONEK
				...
				while (o < count) {
					...
					if (codePoint == 0x3a3) {
						...
						if (codePoint == 0x49) {
							// special case mappings. I becomes dotless i, unless followed by DOT ABOVE
 ...
 * Locale.
 *
 * @return		a new String containing the uppercase characters equivalent to the
 *				characters in this String
 */
...
/**
 * Copies this String removing white space characters from the beginning

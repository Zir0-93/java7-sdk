...
import sun.nio.ch.NetworkProviderHelper;
    ...
    extends SelectableChannel
{
    ...
    private boolean haveValidKeys() {
        synchronized (keyLock) {
            ...
            // RDMA Enabled? Reset already set keys
            if (NetworkProviderHelper.isProviderSwitchDone(this)) {
                this.keyCount = 0;
                this.keys = null;
                return false;  
            }
    ...
    public final SelectionKey keyFor(Selector sel) {
        // RDMA enabled?
        if (NetworkProviderHelper.isProviderSwitchDone(this))
            throw new UnsupportedOperationException("Selector is not supported for RDMA channels");
        return findKey(sel);
    }
    ...
    {
        // RDMA Enabled
        if (!block && NetworkProviderHelper.isProviderSwitchDone(this)) {
            throw new UnsupportedOperationException("Selector is not supported for RDMA channels");
        }
        synchronized (regLock) {
            if (!isOpen())

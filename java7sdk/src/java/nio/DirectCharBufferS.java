    ...
    implements DirectBuffer
{
    ...
    public char get() {
        if (RCMSupport.isRCMSupported() && fd != null) {
            return (Bits.swap(unsafe.rcmGetChar(ix(nextGetIndex()))));
        }
    ...
    public CharBuffer get(char[] dst, int offset, int length) {
        ...
        if ((length << 1) > Bits.JNI_COPY_TO_ARRAY_THRESHOLD) {
            ...
            if (order() != ByteOrder.nativeOrder())
                ...
                if (RCMSupport.isRCMSupported() && fd != null) {
                    Bits.rcmCopyToCharArray(ix(pos), dst,
                                              offset << 1,
                                              length << 1);
                } else {
    ...
    public CharBuffer put(char x) {
        ...
        // add rcm code
        if (RCMSupport.isRCMSupported() && fd != null) {
            unsafe.rcmPutChar(ix(nextPutIndex()), Bits.swap((x)));
        } else {
    ...
    public CharBuffer put(int i, char x) {
        ...
        // add rcm code
        if (RCMSupport.isRCMSupported() && fd != null) {
            unsafe.rcmPutChar(ix(checkIndex(i)), Bits.swap((x)));
        } else {

...
public class URLClassLoader extends SecureClassLoader implements Closeable {
    ...
    /* 
     * IBM multitenancy management value add
     * The TenantContext for multitenancy mode usage
     * It is used to store the TenantContext value for current tenant 
     * */
	...
	ucp = new URLClassPath(urls, null, sharedClassURLClasspathHelper);       //IBM-shared_classes_misc
        this.acc = AccessController.getContext();
        
        /*
         * IBM multitenancy management value add
         * Initialize it only when multitenancy mode enabled
         */
        if (TenantGlobals.isTenantEnabled()){
            this.tenantContext = TenantContext.current();
        }
    }
    ...
    public URLClassLoader(URL[] urls, ClassLoader parent,
        ...
        initializeSharedClassesSupport(urls);                                    //IBM-shared_classes_misc
	ucp = new URLClassPath(urls, factory, sharedClassURLClasspathHelper);    //IBM-shared_classes_misc
    ...
    */
         ...
         throws ClassNotFoundException
    {
        try {
            /*
             * IBM multitenancy management value add
             * in multitenancy mode, tenants are NOT allowed to access objects 
             * created in other tenants' tenant context (called Object Leakage)
             * Here we do a "safe copy" for each tenant to avoid Object Leakage
             * "name" is a final variable, we can not assign new value to it,
             * so modify the original variable name to work around this to minimize the 
             * code merge effort.
             * Please note that this "safe copy" only happened when multitenancy mode
             * is enabled
             */
            final String name = TenantGlobals.isTenantEnabled() ? 
                TenantAccessFactory.getAccess().safeCopy(clsName,this.tenantContext): 
                                    clsName;

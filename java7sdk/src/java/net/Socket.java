...
class Socket implements java.io.Closeable {
    ...
    /**
     * Network provider for this socket.
     * [RDMA_SUPPORT]
     */
    private NetworkProvider netProvider = null;            // RDMA_CAPABILITY
    private boolean providerSwitchDone = false;            // RDMA_CAPABILITY
    ...
    public Socket() {
        setImpl();
        checkAndConfigureKeepAliveForSocket();
    }
    ...
    private Socket(SocketAddress address, SocketAddress localAddr,
        ...
        if (address instanceof InetSocketAddress) {
            setImpl(SocketAction.CONNECT, ((InetSocketAddress)address).getAddress(), ((InetSocketAddress)address).getPort(), true); // RDMA_CAPABILITY
        }
        try {
            createImpl(stream);
            if (stream) {
    ...
    /**
     * Sets impl to the specified network provider.
     * [RDMA SUPPORT]
     */
    void setImpl(NetworkProvider provider, boolean fallback) {               // RDMA_CAPABILITY
    	if (provider != null) {
    		if (providerSwitchDone) {
    			return;
    		}
    		netProvider = provider;
    		impl = netProvider.createImpl(impl);
    		providerSwitchDone = true;
    		if (impl != null) {
    			impl.setSocket(this);
    		}
    	} else {
    ...
    /**
     * Sets impl to the specified IP address and port.
     * [RDMA_SUPPORT]
     */
    void setImpl(SocketAction action, InetAddress address, int port, boolean fallback) {          // RDMA_CAPABILITY
        NetworkProvider provider = null;

    	//provider = NetworkSelector.getNetworkProvider(SocketAction.CONNECT, address, port, null);
    	provider = NetworkSelector.getNetworkProvider(action, address, port, null);
    	setImpl(provider, fallback);
    }                                                                                             // RDMA_CAPABILITY
    ...
    public void connect(SocketAddress endpoint, int timeout) throws IOException {
        ...
        setImpl(SocketAction.CONNECT, addr, port, false);                                          // RDMA_CAPABILITY
    ...
    public void bind(SocketAddress bindpoint) throws IOException {
        ...
        // check whether this is an RDMA bind operation
        setImpl(SocketAction.BIND, addr, port, false);                       // RDMA_CAPABILITY
        if (providerSwitchDone && netProvider.getPreferredAddress() != null) {
               if (addr.isAnyLocalAddress() || addr.isLoopbackAddress()) {
                       addr = netProvider.getPreferredAddress();
               }
        }                                                                    // RDMA_CAPABILITY
    ...
    /**
     * Set the post-accept flags for a specified server socket impl.
     * [RDMA SUPPORT]
     */
    final void postAccept(SocketImpl serverSocketImpl) throws IOException {    // RDMA_CAPABILITY
    	if (netProvider != null) {
    		netProvider.postAccept(serverSocketImpl, this.impl);
    	}
    	postAccept();
    }                                                                          // RDMA_CAPABILITY
    ...
    /**
     * Check and configure the KeepAlive Parameters for the socket
     * if KeepAlive properties are set for this thread
     * Note: This code needs to be revisited if and when RDMA
     * supports KeepAlive parameters
     */
    private void checkAndConfigureKeepAliveForSocket() {

        try {
            // Get the properties set for this thread
            ThreadPropertyMap threadPropertyMap = 
                (ThreadPropertyMap) AccessController.doPrivileged(
            ...
            if (threadPropertyMap != null) {
                String idleTime = 
                    threadPropertyMap.get(ThreadProperties.KEEPALIVE_IDLE_TIME);
                String probeCount = 
                    threadPropertyMap.get(ThreadProperties.KEEPALIVE_PROBE_COUNT);
                String intervalTime = 
                    threadPropertyMap.get(ThreadProperties.KEEPALIVE_INTERVAL_TIME);

                if (idleTime != null || probeCount != null || intervalTime != null) {
                    SocketKeepAliveParameters keepAliveParameters = 
                        new SocketKeepAliveParameters();
                    if (idleTime != null) {
                        keepAliveParameters.setIdleTime(Integer.parseInt(idleTime));
                    }
                    if (probeCount != null) {
                        keepAliveParameters.setProbeCount(Integer.parseInt(probeCount));
                    }
                    if (intervalTime != null) {
                        keepAliveParameters.setIntervalTime(Integer.parseInt(intervalTime));
                    }
                    // enable SO_KEEPALIVE
                    this.setKeepAlive(true);
                    // configure KeepAlive parameters
                    SocketUtils.configureKeepAlive(this, keepAliveParameters);
                }
            }
        } catch (NumberFormatException nfe) {
            throw new IllegalArgumentException("KeepAlive ThreadProperties " + 

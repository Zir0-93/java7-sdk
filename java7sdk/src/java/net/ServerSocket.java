...
class ServerSocket implements java.io.Closeable {
    ...
    public ServerSocket(int port, int backlog, InetAddress bindAddr) throws IOException {
        setImpl(bindAddr, port, true);                     // RDMA_CAPABILITY
     ...
     * When there is no network provider, we invoke the default socket
     * impl only if the fallback option is true.
     * [RDMA_SUPPORT]
     */
    private void setImpl(NetworkProvider provider, boolean fallback) {         // RDMA_CAPABILITY
    	if (provider != null) {
    ...
    /**
     * Set impl for the specified IP address and port.
     * [RDMA_SUPPORT]
     */
    private void setImpl(InetAddress address, int port, boolean fallback) {    // RDMA_CAPABILITY
    	NetworkProvider provider = null;
		if (address == null) {
			address = (new InetSocketAddress(port)).getAddress();
		}
    	provider = NetworkSelector.getNetworkProvider(SocketAction.BIND, address, port, null);
    ...
    public void bind(SocketAddress endpoint, int backlog) throws IOException {
        ...
        setImpl(epoint.getAddress(), epoint.getPort(), false);               // RDMA_CAPABILITY
        if (providerSwitchDone && netProvider.getPreferredAddress() != null) {
        	InetAddress addr = epoint.getAddress();
        	if (addr.isAnyLocalAddress() || addr.isLoopbackAddress()) {
        		epoint = new InetSocketAddress(netProvider.getPreferredAddress(), epoint.getPort());
        	}
        }                                                                    // RDMA_CAPABILITY
    ...
    protected final void implAccept(Socket s) throws IOException {
        ...
        try {
			...
			/* RDMAServerSocket should typically accept RDMASocket             // RDMA_CAPABILITY
 			 * unless TCP fallback mode is enabled, in which case
 			 * TCP clients can also connect to RDMA server
 			 */
			if (providerSwitchDone) {
				s.setImpl(netProvider, false);
			}                                                                  // RDMA_CAPABILITY
            si = s.impl;
            s.impl = null;
			...
			/* Check whether remote host is allowed to connect to this server
 			 * in RDMA mode
 			 */
			if (providerSwitchDone) {
            	NetworkProvider provider = NetworkSelector.getNetworkProvider(SocketAction.ACCEPT, getInetAddress(),
            			getLocalPort(), si.getInetAddress());                      // RDMA_CAPABILITY
            	if (provider == null) {
					si.close();

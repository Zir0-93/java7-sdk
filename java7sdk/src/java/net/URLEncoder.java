...
public class URLEncoder {
	...
	int maxBytesPerChar = 10; // rather arbitrary limit, but safe for now
      StringBuffer out = new StringBuffer(s.length()*3/2);
	ByteArrayOutputStream buf = new ByteArrayOutputStream(maxBytesPerChar);

	ResettableBufferedOutputStreamWriter writer = 
						new ResettableBufferedOutputStreamWriter(buf, enc); 
	...
	for (int i = 0; i < s.length(); i++) {
		...
		for (int j = 0; j < ba.length; j++) {
		    out.append('%');
		    out.append(hexStr.charAt((ba[j] >> 4) & 0x0f));  
                out.append(hexStr.charAt(ba[j]&0x0f));           
		}
		buf.reset();
		needToChange = true;
	    }

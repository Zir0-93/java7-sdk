...
public class Manifest implements Cloneable {
    ...
    static class FastInputStream extends FilterInputStream {
        ...
        FastInputStream(InputStream in) {
           /* We localize the InputStream since for z/OS the input will normally be //IBM-zos_bringup
            * EBCDIC and the readLine method of this class makes the assumption that //IBM-zos_bringup
            * the input is ASCII. We could mandate that manifest files must always //IBM-zos_bringup
            * be ASCII encoded; however this makes it difficult to write and maintain //IBM-zos_bringup
            * manifest files on z/OS. Also the build creates a manifest from a text //IBM-zos_bringup
            * file and edits using sed which work on EBCDIC data, for z/OS, hence we //IBM-zos_bringup
            * would otherwise need to fix up the build.                         //IBM-zos_bringup
             */                                                                 //IBM-zos_bringup
           this(LocalizedInputStream.localize(in), 8192);                       //IBM-zos_bringup
                                                                                //IBM-zos_bringup
        }

        FastInputStream(InputStream in, int size) {
           super(LocalizedInputStream.localize(in));                            //IBM-zos_bringup
            buf = new byte[size];
        }

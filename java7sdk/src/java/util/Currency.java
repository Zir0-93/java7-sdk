...
public final class Currency implements Serializable {
    ...
    public static Currency getInstance(Locale locale) {
        ...
        String language = locale.getLanguage();                                 //IBM-euro_currency_oracle_giveback
                                                                                //IBM-euro_currency_oracle_giveback
        String variant = locale.getVariant();                                   //IBM-euro_currency_oracle_giveback
                                                                                //IBM-euro_currency_oracle_giveback
        /* The getNumberFormatData(locale) is used here to get the FormatData's resource bundle //IBM-euro_currency_oracle_giveback
           from which the value of the Currency property can be retrieved.      //IBM-euro_currency_oracle_giveback
           getNumberFormatData(locale) is not specific to NumberFormat. It returns the //IBM-euro_currency_oracle_giveback
           FormatData's resource bundle for the given locale and so is used here. */ //IBM-euro_currency_oracle_giveback
        if ("EURO".equals(variant) || "PREEURO".equals(variant))  {             //IBM-euro_currency_oracle_giveback
            ResourceBundle rb = LocaleData.getNumberFormatData(locale);         //IBM-euro_currency_oracle_giveback
            try{                                                                //IBM-euro_currency_oracle_giveback
                String CurrencyCode = rb.getString("Currency");                 //IBM-euro_currency_oracle_giveback
                return getInstance(CurrencyCode);                               //IBM-euro_currency_oracle_giveback
            } catch(MissingResourceException e) {                               //IBM-euro_currency_oracle_giveback
                return getInstance(new Locale(language,country));               //IBM-euro_currency_oracle_giveback
            }                                                                   //IBM-euro_currency_oracle_giveback
        }                                                                       //IBM-euro_currency_oracle_giveback
                                                                                //IBM-euro_currency_oracle_giveback
        if (country == null) {

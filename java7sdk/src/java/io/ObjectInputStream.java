    ...
    extends InputStream implements ObjectInput, ObjectStreamConstants
{
    ...
    static {
     ...
     /** if true LUDCL/forName results would be cached, false by default */
    ...
    /** LUDCL results cached based on the property enableClassCaching */
    private ClassLoader cachedLudcl;

	private static final byte TC_PACKED = (byte) 0x99;

	/** count the depth of entering readPackedObject*/
	private int readPackedDepth = 0;
    ...
    public ObjectInputStream(InputStream in) throws IOException {
        if(in instanceof com.ibm.jvm.io.ConsoleInputStream){                    //IBM-console_io
            ((com.ibm.jvm.io.ConsoleInputStream)in).setConversionRequired(false); //IBM-console_io
        }                                                                       //IBM-console_io
        verifySubclass();
	...
	if ((curContext == null) && (isClassCachingEnabled)) {
        ...
        try {
            ...
            // skip mark dependency if the current readObject is inside
            // readPackedObject method, PO solve the dependency itself
            if (readPackedDepth == 0) {
    ...
    private Object readObject0(boolean unshared) throws IOException {
		...
		case TC_PACKED:
			return checkResolve(readPackedObject());
                case TC_EXCEPTION:
	...
	private Object readPackedObject() throws IOException
	{
		readPackedDepth++;
		bin.readByte();
		Object obj;
		try {
			obj = (Object) PackedObjectSupport.readPackedObject(this);
			return obj;
		} catch (ClassNotFoundException | InstantiationException e) {
			throw new IOException(String.format("Unhandled Exception while readPackedObject %s ", e.getMessage()));

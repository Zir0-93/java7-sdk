...
public class ObjectStreamClass implements Serializable {
    ...
    private static final boolean isPacked = 
        (Boolean) AccessController.doPrivileged( 
            new PrivilegedAction() {
                public Object run() {
                	return com.ibm.oti.vm.VM.PACKED_SUPPORT_ENABLED ;
                }
            }
        );
    ...
    private static class Caches {
        ...
        static final ConcurrentMap<FieldReflectorKey,Reference<?>> reflectors =
            new ConcurrentHashMap<>();
		                                                                //IBM-T6_wrt_bringup
		/** cache for RealTime */                                       //IBM-T6_wrt_bringup
		static final ConcurrentMap localDescsRT = new ConcurrentHashMap(); //IBM-T6_wrt_bringup
		                                                                //IBM-T6_wrt_bringup
    ...
    static ObjectStreamClass lookup(Class<?> cl, boolean all) {
		...
		{                                                               //IBM-T6_wrt_bringup
			return lookupforRT(cl);                                 //IBM-T6_wrt_bringup
		}                                                               //IBM-T6_wrt_bringup
	    processQueue(Caches.localDescsQueue, Caches.localDescs);            //IBM-T6_wrt_bringup
	...
	static ObjectStreamClass lookupforRT(Class cl)                          //IBM-T6_wrt_bringup
	{                                                                       //IBM-T6_wrt_bringup
        Object entry = Caches.localDescsRT.get(cl);                             //IBM-T6_wrt_bringup
        EntryFuture future = null;                                              //IBM-T6_wrt_bringup
        if (entry == null) {                                                    //IBM-T6_wrt_bringup
			//future = new EntryFuture();                           //IBM-T6_wrt_bringup
			EntryFuture newEntry = new EntryFuture();               //IBM-T6_wrt_bringup
			entry = Caches.localDescsRT.putIfAbsent(cl, newEntry);  //IBM-T6_wrt_bringup
			if(entry == null) {                                     //IBM-T6_wrt_bringup
				future = newEntry;                              //IBM-T6_wrt_bringup
			}                                                       //IBM-T6_wrt_bringup
		}                                                               //IBM-T6_wrt_bringup
                                                                                //IBM-T6_wrt_bringup
        if (entry instanceof ObjectStreamClass) {  // check common case first   //IBM-T6_wrt_bringup
            return (ObjectStreamClass) entry;                                   //IBM-T6_wrt_bringup
        }                                                                       //IBM-T6_wrt_bringup
        if (entry instanceof EntryFuture) {                                     //IBM-T6_wrt_bringup
            future = (EntryFuture) entry;                                       //IBM-T6_wrt_bringup
            if (future.getOwner() == Thread.currentThread()) {                  //IBM-T6_wrt_bringup
                /*                                                              //IBM-T6_wrt_bringup
                 * Handle nested call situation described by 4803747: waiting   //IBM-T6_wrt_bringup
                 * for future value to be set by a lookup() call further up the //IBM-T6_wrt_bringup
                 * stack will result in deadlock, so calculate and set the      //IBM-T6_wrt_bringup
                 * future value here instead.                                   //IBM-T6_wrt_bringup
                 */                                                             //IBM-T6_wrt_bringup
                entry = null;                                                   //IBM-T6_wrt_bringup
            } else {                                                            //IBM-T6_wrt_bringup
                entry = future.get();                                           //IBM-T6_wrt_bringup
            }                                                                   //IBM-T6_wrt_bringup
        }                                                                       //IBM-T6_wrt_bringup
        if (entry == null) {                                                    //IBM-T6_wrt_bringup
            try {                                                               //IBM-T6_wrt_bringup
                entry = new ObjectStreamClass(cl);                              //IBM-T6_wrt_bringup
            } catch (Throwable th) {                                            //IBM-T6_wrt_bringup
                entry = th;                                                     //IBM-T6_wrt_bringup
            }                                                                   //IBM-T6_wrt_bringup
            if (future.set(entry)) {                                            //IBM-T6_wrt_bringup
                Caches.localDescsRT.put(cl, entry);                             //IBM-T6_wrt_bringup
            } else {                                                            //IBM-T6_wrt_bringup
                // nested lookup call already set future                        //IBM-T6_wrt_bringup
                entry = future.get();                                           //IBM-T6_wrt_bringup
            }                                                                   //IBM-T6_wrt_bringup
        }                                                                       //IBM-T6_wrt_bringup
                                                                                //IBM-T6_wrt_bringup
        if (entry instanceof ObjectStreamClass) {                               //IBM-T6_wrt_bringup
            return (ObjectStreamClass) entry;                                   //IBM-T6_wrt_bringup
        } else if (entry instanceof RuntimeException) {                         //IBM-T6_wrt_bringup
            throw (RuntimeException) entry;                                     //IBM-T6_wrt_bringup
        } else if (entry instanceof Error) {                                    //IBM-T6_wrt_bringup
            throw (Error) entry;                                                //IBM-T6_wrt_bringup
        } else {                                                                //IBM-T6_wrt_bringup
            throw new InternalError("unexpected entry: " + entry);              //IBM-T6_wrt_bringup
        }                                                                       //IBM-T6_wrt_bringup
    }                                                                           //IBM-T6_wrt_bringup
    /**
    ...
    boolean isPackedObject(Object po)
    {
    	if(isPacked)
    	{
    		 if (po instanceof PackedObject) 
    		{
    			return true;
    		}
    	}
